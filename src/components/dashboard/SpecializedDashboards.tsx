...
import {
  TrendingUp, MessageSquare, Sparkles, Clock, CheckCircle2, AlertCircle,
  Users, Target, Zap, Calendar, Timer, Bell, Star, RefreshCw, DollarSign,
  BarChart3, PieChart, ArrowUpRight, ArrowDownRight, FileText, Send,
  Instagram, Facebook, Linkedin, Youtube, Megaphone, Video, Film,
  PenTool, Type, Copy, Wand2, UserPlus, Briefcase, Award, Heart,
  Building2, Phone, Mail, Globe, ShoppingCart, CreditCard, Receipt,
  Wallet, Banknote, Calculator, UserCheck, CalendarDays, Gift, Cake,
  Activity, Gauge, Eye, TrendingDown, Play, Folder
} from 'lucide-react';
...
export const CopywritingDashboard = ({ config }: { config: any }) => {
  const [currentInsightIndex, setCurrentInsightIndex] = useState(0);
  ...
  const recentTexts = [
    { id: 1, title: 'Email Marketing - Black Friday', client: 'Tech Solutions', type: 'email', status: 'approved', words: 450 },
    { id: 2, title: 'Landing Page - Produto Novo', client: 'E-commerce Plus', type: 'landing', status: 'review', words: 800 },
    { id: 3, title: 'Posts Instagram - Dezembro', client: 'Marketing Pro', type: 'social', status: 'draft', words: 320 }
  ];

  const templates = [
    { name: 'Email Promocional', uses: 45, icon: Mail },
    { name: 'Post Instagram', uses: 78, icon: Instagram },
    { name: 'Headline Ads', uses: 32, icon: Megaphone },
    { name: 'Landing Page', uses: 23, icon: Globe }
  ];

  const valInsights = [
    { icon: '✍️', title: 'Produtividade', message: 'Você escreveu 12.5K palavras esta semana. 15% acima da média!' },
    { icon: '💡', title: 'Dica de Copy', message: 'CTAs com urgência têm 25% mais cliques. Experimente "Últimas vagas".' },
    { icon: '🎯', title: 'Performance', message: 'Seus emails para Tech Solutions têm 45% de abertura. Excelente!' },
    { icon: '📝', title: 'Revisão', message: '5 textos aguardando aprovação. O mais antigo tem 2 dias.' }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold">Painel do Copywriter</h2>
              <p className="text-white/80">Crie textos persuasivos e acompanhe entregas</p>
            </div>
          </div>
          <div className="hidden md:flex items-center gap-3">
            <div className="px-4 py-2 bg-white/20 rounded-xl backdrop-blur-md">
              <p className="text-xs text-white/70">Palavras esta semana</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Textos Semana', value: stats.textsThisWeek, icon: FileText, color: '#f97316' },
          { label: 'Em Revisão', value: stats.pendingReview, icon: Clock, color: '#eab308' },
          { label: 'Taxa Aprovação', value: stats.avgApprovalRate, icon: CheckCircle2, color: '#10b981' },
          { label: 'Palavras', value: stats.wordsWritten, icon: Type, color: '#8b5cf6' }
        ].map((stat, idx) => (
          <motion.div
            key={idx}
            initial={{ opacity: 0, scale: 0.9 }}
        ...
        >
          ...
          <div className="space-y-3">
            {recentTexts.map((text) => (
              <div key={text.id} className="p-4 rounded-xl bg-gray-50 border border-gray-100 hover:shadow-md transition-all cursor-pointer">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <p className="font-medium text-gray-800">{text.title}</p>
                    <p className="text-xs text-gray-500">{text.client} • {text.words} palavras</p>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded-full ${
                    text.status === 'approved' ? 'bg-green-100 text-green-700' :
                    text.status === 'review' ? 'bg-orange-100 text-orange-700' :
                    'bg-gray-100 text-gray-600'
                  }`}>
                    {text.status === 'approved' ? 'Aprovado' : text.status === 'review' ? 'Em Revisão' : 'Rascunho'}
                  </span>
                </div>
              </div>
            ))}
        ...
        >
          ...
          <div className="space-y-3">
            {templates.map((template, idx) => (
              <div key={idx} className="p-3 rounded-lg bg-gray-50 flex items-center justify-between hover:bg-gray-100 cursor-pointer transition-all">
                <div className="flex items-center gap-3">
                  <template.icon className="w-5 h-5 text-gray-600" />
                  <span className="text-sm font-medium text-gray-800">{template.name}</span>
                </div>
                <span className="text-xs text-gray-500">{template.uses}x</span>
              </div>
            ))}
          </div>
        </motion.div>
...
// ==================== GESTÃO/CEO DASHBOARD ====================
export const GestaoDashboard = ({ config }: { config: any }) => {
  const stats = {
    mrr: 'R$ 285K',
    clients: 42,
    nps: 72,
  ...
  const kpis = [
    { name: 'Receita Mensal', value: 'R$ 285K', change: '+12%', positive: true },
    { name: 'Novos Clientes', value: '8', change: '+3', positive: true },
    { name: 'Churn', value: '2.1%', change: '-0.5%', positive: true },
    { name: 'Ticket Médio', value: 'R$ 6.8K', change: '+8%', positive: true }
  ];

  const departments = [
    { name: 'Marketing', tasks: 45, completed: 38, health: 84 },
    { name: 'Comercial', tasks: 28, completed: 22, health: 79 },
    { name: 'Financeiro', tasks: 15, completed: 14, health: 93 },
    { name: 'RH', tasks: 12, completed: 10, health: 83 }
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/80">Visão 360° da empresa</p>
            </div>
          </div>
          <div className="hidden md:flex items-center gap-3">
            <div className="px-4 py-2 bg-white/20 rounded-xl backdrop-blur-md">
            ...
            <div className="px-4 py-2 bg-green-500/30 rounded-xl backdrop-blur-md">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { label: 'MRR', value: stats.mrr, icon: DollarSign, color: '#1e293b' },
          { label: 'Clientes', value: stats.clients, icon: Building2, color: '#3b82f6' },
          { label: 'NPS', value: stats.nps, icon: Star, color: '#f59e0b' },
          { label: 'Equipe', value: stats.teamSize, icon: Users, color: '#8b5cf6' },
          { label: 'Crescimento', value: stats.growth, icon: TrendingUp, color: '#10b981' }
        ].map((stat, idx) => (
          <motion.div
            key={idx}
            initial={{ opacity: 0, scale: 0.9 }}
            ...
            <div key={idx} className="p-4 rounded-lg bg-gray-50">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-sm font-bold ${
                  dept.health >= 90 ? 'text-green-600' : dept.health >= 70 ? 'text-yellow-600' : 'text-red-600'
                }`}>
                  {dept.health}%
                </span>
              </div>
              <div className="h-2 bg-gray-200 rounded-full mb-2">
                <div 
                  className={`h-2 rounded-full ${
                    dept.health >= 90 ? 'bg-green-500' : dept.health >= 70 ? 'bg-yellow-500' : 'bg-red-500'
                  }`}
                  style={{ width: `${dept.health}%` }}
                />
              </div>
              <p className="text-xs text-gray-500">{dept.completed}/{dept.tasks} tarefas concluídas</p>
            </div>
          ))}
        </div>
      </motion.div>
